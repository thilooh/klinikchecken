    ...
    <>
      ...
      >
        ...
        <button
          ...
          style={{ position: 'absolute', top: showCertifiedBadge ? '26px' : '10px', right: '10px', background: 'none', border: 'none', cursor: 'pointer', padding: '4px', zIndex: 1 }}
        >
        ...
        <div style={{ display: 'flex', gap: '12px', marginTop: showCertifiedBadge ? '14px' : '0' }}>
          ...
          {/* Col 3: Methods – desktop only */}
          <div className="hidden sm:block" style={{ minWidth: '150px', maxWidth: '190px' }}>
              ...
              {clinic.tags.slice(0, 3).map(tag => {
                ...
                return (
                  <span
                    ...
                    style={{ backgroundColor: isActive ? '#003399' : '#F0F0F0', color: isActive ? '#fff' : '#444', fontSize: '11px', padding: '3px 7px', borderRadius: '4px', cursor: isMethod ? 'pointer' : 'default', transition: 'background 0.15s' }}
                  >
                    {tag}
                  </span>
                )
            ...
            {clinic.onlineBooking && (
              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#00A651', fontSize: '12px', marginBottom: '4px' }}>
                <Clock size={11} />Online-Buchung
              </div>
            )}
            {clinic.photoCount > 0 && (
              <a href="#" style={{ display: 'flex', alignItems: 'center', gap: '4px', color: '#003399', fontSize: '12px', textDecoration: 'none' }}>
                <ExternalLink size={11} />{clinic.photoCount} Praxis-Fotos
              </a>
            )}
          </div>

          {/* Col 4: Price + CTA – desktop only */}
          <div className="hidden sm:flex flex-col justify-between" style={{ flexShrink: 0, textAlign: 'right', minWidth: '148px' }}>
            <div>
