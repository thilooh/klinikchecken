  ...
  return (
    <div style={{
      ...
      <div className="max-w-[1200px] mx-auto px-4">
        ...
        {/* Trust pills – top */}
        <div style={{ display: 'flex', justifyContent: 'center', flexWrap: 'wrap', gap: '8px', marginBottom: '14px' }}>
          {['Geprüfte Ärzte & Kliniken', 'SSL-gesichert', 'Kostenlos & unverbindlich'].map(label => (
            <span key={label} style={{ display: 'flex', alignItems: 'center', gap: '4px', backgroundColor: 'rgba(255,255,255,0.1)', color: '#C2D8F5', fontSize: '11px', padding: '4px 10px', borderRadius: '20px', fontWeight: 500 }}>
              <CheckCircle2 size={10} color="#5B9BFF" />
              {label}
            </span>
          ))}
        </div>
        ...
        <p style={{ color: '#7AAAE0', fontSize: '13px', textAlign: 'center', marginBottom: '18px', fontWeight: 400 }}>
        ...
        {/* Trust footer */}
        <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '18px', marginTop: '14px', flexWrap: 'wrap' }}>
          {[
            { n: '500+', label: 'geprüfte Anbieter' },
            { n: '12.000+', label: 'Bewertungen' },
            { n: '2', label: 'Städte verfügbar' },
          ].map(stat => (
            <div key={stat.n} style={{ textAlign: 'center' }}>
              <div style={{ color: '#fff', fontWeight: 800, fontSize: '14px', lineHeight: 1 }}>{stat.n}</div>
              <div style={{ color: '#7AAAE0', fontSize: '10px', marginTop: '2px' }}>{stat.label}</div>
            </div>
          ))}
        </div>
